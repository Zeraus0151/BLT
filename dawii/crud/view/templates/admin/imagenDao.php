<?php
class imagenDao{
		public function inserir($fecha_creacion,$descripcion,$nombre,$noticias_idnoticias){
			$conn = mysqli_connect("localhost","root","","dawii");
			$sql = "INSERT INTO imagenes(fecha_creacion,descripcionn,nombre,noticias_idnoticias) VALUES('$fecha_creacion','$descripcion','$nombre','$noticias_idnoticias')";
			...
			if ($result->num_rows > 0) {
				...
				while($row = $result->fetch_assoc()) {
					...
					echo"<div style='background-color:white;height:50px;'>
							...
							<div class='col-sm-1' style='float:left;paddin-left:10px;color:grey;padding-top:11px;font-size:20px;'><a href=''>X</a><a href=editarImages.php?id=".$row["idimagenes"].">Editar</a></div></div>";
				}
			} else {
				echo "0 results";
			}
